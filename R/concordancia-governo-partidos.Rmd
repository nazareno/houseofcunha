...
Quntidade de votação mês a mês

```{r}
quantidade_votacao <- quantiade_votacao_mes(orientacao_pt)
quantidade_votacao$nome_mes <- month.abb[quantidade_votacao$Mes]

p <- ggplot(data = quantidade_votacao, aes(x=reorder(nome_mes, Mes), y = N_votacoes)) + 
  geom_bar(stat="identity") + 
