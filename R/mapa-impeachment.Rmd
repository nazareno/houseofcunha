...
doacao_lava_jato <- read.table("dados/doacao_lava_jato.csv", header = TRUE, quote = "\"")
...
p <- plotCluster(mca_obs_df) + 
  ...
  geom_point(data = filter(mca_obs_df, destaque_investigados == TRUE), 
             shape = 18, size = 10, alpha = 0.5, colour = "Black") +
  geom_text(data = filter(mca_obs_df, destaque_investigados == TRUE), 
                   aes(x = Dim.1, y = Dim.2, label = paste(nome, "-", toupper(partido))),
                   colour = "black", alpha = 0.6, size = 5, hjust = 0.5, vjust = 2) +
  coord_equal() 
...
Plot dos deputados que respondem processo no stf 
...
Plot dos deputados que receberam doações de empresas da lava jato
