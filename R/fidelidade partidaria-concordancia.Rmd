...
concordancia_governo <- data.frame(Concordancia = c(pr, ptn, pp, prb, psd), Partidos = c("pr", "ptn", "pp", "prb", "psd"))

str(concordancia_governo)

p <- ggplot(data = concordancia_governo, aes(x=reorder(Partidos, -Concordancia), y = Concordancia, fill = "#fec44f")) + 
...
png("plot/fidelidade/concordancia_governo_partidos_3.png", width = 900, height = 500)
p
dev.off()
```
...
mca_obs_df$destaque_partido = factor(ifelse(mca_obs_df$partido %in% 
                                               c("pr", "pt",  "pp", "psd", "ptn", "prb"), 
                                             as.character(mca_obs_df$partido), 
                                             "outros"))  

p <- ggplot(data = mca_obs_df, 
       aes(x = Dim.1, y = Dim.2, label = partido, color = destaque_partido)) +
...
png("plot/fidelidade/mapa_centrao_1.png", width = 960, height = 960)
p
dev.off()
