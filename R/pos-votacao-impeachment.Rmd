...
match_deputados <- function(nome.deputados.novo, nome.deputados.original){
  ...
  for (nome in nome.deputados.novo){
    novo_nome <- agrep(nome, nome.deputados.original, ignore.case=T, value=T, max.distance = 0.1, useBytes = FALSE)
...
frequencia_palavra_discursos <- function (discursos){
...
frequencia.discursos.sim <- frequencia_palavra_discursos(discursos.sim)
frequencia.discursos.nao <- frequencia_palavra_discursos(discursos.nao)
...
frequencia.discursos.sim.nao[1:100,]

frequencia.discursos.sim.nao.sample <- frequencia.discursos.sim.nao[1:200,]


ggplot(data = frequencia.discursos.sim.nao.sample, aes(x = top, y = top_nao)) + 
   #geom_point(size = 9, alpha = 0.3) + 
  geom_text(data = frequencia.discursos.sim.nao.sample, 
                   aes(x = top, y = top_nao, label = Palavra),
